...
# 投诉/升级关键词（情感激烈用户经常说）
COMPLAINT_KEYWORDS = ["投诉", "起诉", "曝光", "315", "消协", "工商局", "黑猫"]


def detect_forced_tool(user_content: str, emotion: dict | None = None):
    """检测用户意图是否明确要求某个工具，返回 tool_choice 覆盖配置

    Args:
        user_content: 用户消息
        emotion: 情绪检测结果（可选），强情绪会触发强制转人工

    返回值：
        - dict {"type":"function","function":{"name":"..."}}：强制调某工具
        - "required"：强制调任意工具
        - None：不干预，让模型自由判断
    """
    # 1. 转人工 / 投诉关键词 → 最强信号
    ...
    if any(kw in user_content for kw in COMPLAINT_KEYWORDS):
        return {"type": "function", "function": {"name": "escalate_to_human"}}

    # 2. 强情绪（intensity >= 8）→ 强制转人工兜底
    if emotion and emotion.get("intensity", 0) >= 8:
        return {"type": "function", "function": {"name": "escalate_to_human"}}

    has_order = bool(ORDER_PATTERN.search(user_content))
    has_refund_intent = any(kw in user_content for kw in ["退货", "退掉", "退款", "不要了", "退一下", "退钱"])

    # 3. 退货 + 订单号 → 强制调（先查后退是 AI 的事）
...
- **🚨 绝不要编造电话号码** —— 如知识库写"400-XXX-XXXX"就原样保留 X，不要换成具体数字。需要给电话时一律说"具体联系方式请查看 CC 商城官网底部"
- **如果工具返回里有 `ai_response_template` 字段，必须按该模板回复，不要自由发挥添加细节**
...
def chat_stream(
    ...
    if skip_rag:
        ...
        # emotion 也参与判定：强情绪自动强制 escalate_to_human
        forced_tool_choice = detect_forced_tool(user_content, emotion=emotion)
